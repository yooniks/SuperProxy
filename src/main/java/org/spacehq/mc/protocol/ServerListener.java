...
public class ServerListener extends SessionAdapter {
  ...
  public void packetReceived(final PacketReceivedEvent event) {
    ...
    if (protocol.getSubProtocol() == SubProtocol.HANDSHAKE && event
        .getPacket() instanceof HandshakePacket) {
      ...
      switch (packet.getIntent()) {
        ...
        case LOGIN: {
          ...
          if (packet.getProtocolVersion() > 47) {
            event.getSession().disconnect("Outdated listener! I'm still on 1.8.8.");
            break;
          }
          if (packet.getProtocolVersion() < 47) {
    ...
    if (protocol.getSubProtocol() == SubProtocol.LOGIN) {
      if (event.getPacket() instanceof LoginStartPacket) {
        ...
        final boolean verify =
            !event.getSession().hasFlag("verify-users") || (boolean) event.getSession()
                .getFlag("verify-users");
        if (verify) {
          event.getSession().send(
              new EncryptionRequestPacket(this.serverId, ServerListener.KEY_PAIR.getPublic(),
                  this.verifyToken));
        } else {
    ...
    if (protocol.getSubProtocol() == SubProtocol.STATUS) {
      if (event.getPacket() instanceof StatusQueryPacket) {
        ...
        if (builder == null) {
          event.getSession().disconnect("No listener info builder set.");
          return;
        }
        ...
        event.getSession()
            .send(new StatusPongPacket(((StatusPingPacket) event.getPacket()).getPingTime()));
      }
    }
    if (protocol.getSubProtocol() == SubProtocol.GAME && event
        .getPacket() instanceof ClientKeepAlivePacket) {
  ...
  private class UserAuthTask implements Runnable {
    ...
    public void run() {
      final boolean verify =
          !this.session.hasFlag("verify-users") || (boolean) this.session.getFlag("verify-users");
      ...
      if (verify && this.key != null) {
        ...
        try {
          profile = new SessionService(proxy).getProfileByServer(ServerListener.this.username,
              new BigInteger(CryptUtil.getServerIdHash(ServerListener.this.serverId,
                  ServerListener.KEY_PAIR.getPublic(), this.key)).toString(16));
        ...
        profile = new GameProfile(
            UUID.nameUUIDFromBytes(("OfflinePlayer:" + ServerListener.this.username).getBytes()),
            ServerListener.this.username);
      }
