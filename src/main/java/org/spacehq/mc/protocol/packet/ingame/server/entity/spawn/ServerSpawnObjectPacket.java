...
public class ServerSpawnObjectPacket implements Packet {
  ...
  public ServerSpawnObjectPacket(final int entityId, final ObjectType type, final double x,
      final double y, final double z, final float yaw, final float pitch) {
    this(entityId, type, null, x, y, z, yaw, pitch, 0.0, 0.0, 0.0);
  }

  public ServerSpawnObjectPacket(final int entityId, final ObjectType type, final ObjectData data,
  ...
  public ServerSpawnObjectPacket(final int entityId, final ObjectType type, final double x,
      final double y, final double z, final float yaw, final float pitch, final double motX,
  ...
  public ServerSpawnObjectPacket(final int entityId, final ObjectType type, final ObjectData data,
      final double x, final double y, final double z, final float yaw, final float pitch,
      final double motX, final double motY, final double motZ) {
  ...
  public void read(final NetInput in) throws IOException {
    ...
    if (data > 0) {
      ...
      } else if (this.type == ObjectType.ARROW || this.type == ObjectType.BLAZE_FIREBALL
          || this.type == ObjectType.FISH_HOOK || this.type == ObjectType.GHAST_FIREBALL
          || this.type == ObjectType.WITHER_HEAD_PROJECTILE) {
  ...
  public void write(final NetOutput out) throws IOException {
    ...
    if (this.data != null) {
      if (this.data instanceof MinecartType) {
        ...
        data = (((FallingBlockData) this.data).getId()
            | ((FallingBlockData) this.data).getMetadata() << 16);
