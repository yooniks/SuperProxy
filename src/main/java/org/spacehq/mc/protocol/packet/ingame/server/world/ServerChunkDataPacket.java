...
public class ServerChunkDataPacket implements Packet {
  ...
  public ServerChunkDataPacket(final int x, final int z, final Chunk[] chunks,
    ...
    if (noSkylight && skylight) {
      throw new IllegalArgumentException(
          "Either all chunks must have skylight values or none must have them.");
    }
    this.x = x;
    this.z = z;
    this.chunks = chunks;
  ...
  public void read(final NetInput in) throws IOException {
    ...
    final ParsedChunkData chunkData = NetUtil
        .dataToChunks(new NetworkChunkData(chunkMask, fullChunk, false, data), true);
  ...
  public void write(final NetOutput out) throws IOException {
    final NetworkChunkData data = NetUtil
        .chunksToData(new ParsedChunkData(this.chunks, this.biomeData));
    out.writeInt(this.x);
    out.writeInt(this.z);
