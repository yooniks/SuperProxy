      ...
      public void run() {
        for (final Set<String> request : partition(criteria, 100)) {
          ...
          while (failCount < 3 && tryAgain) {
            tryAgain = false;
            try {
              final GameProfile[] profiles = HTTP.makeRequest(ProfileService.this.proxy,
                  "https://api.mojang.com/profiles/minecraft", request, GameProfile[].class);
              failCount = 0;
              ...
              for (final String name : missing) {
                callback.onProfileLookupFailed(new GameProfile((UUID) null, name),
                    new ProfileNotFoundException("Server could not find the requested profile."));
              }
              try {
                Thread.sleep(100L);
