...
public class ServerScoreboardObjectivePacket implements Packet {
  ...
  public ServerScoreboardObjectivePacket(final String name, final ObjectiveAction action,
      final String displayName, final ScoreType type) {
    if (action != ObjectiveAction.ADD && action != ObjectiveAction.UPDATE) {
      throw new IllegalArgumentException(
          "(name, action, displayName) constructor only valid for adding and updating objectives.");
    }
    this.name = name;
    this.action = action;
