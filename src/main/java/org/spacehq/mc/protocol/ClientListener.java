    ...
    if (protocol.getSubProtocol() == SubProtocol.LOGIN) {
      if (event.getPacket() instanceof EncryptionRequestPacket) {
        ...
        final String serverHash = new BigInteger(
            CryptUtil.getServerIdHash(packet.getServerId(), packet.getPublicKey(), key))
            .toString(16);
        ...
        try {
          ...
          event.getSession()
              .disconnect("Login failed: Authentication error: " + e3.getMessage(), e3);
          return;
        }
        event.getSession().send(
            new EncryptionResponsePacket(key, packet.getPublicKey(), packet.getVerifyToken()));
        ...
        event.getSession().setCompressionThreshold(
            ((LoginSetCompressionPacket) event.getPacket()).getThreshold());
      }
      ...
      if (event.getPacket() instanceof StatusResponsePacket) {
        ...
        final ServerInfoHandler handler = event.getSession().getFlag("listener-info-handler");
        if (handler != null) {
        ...
        final long time =
            System.currentTimeMillis() - ((StatusPongPacket) event.getPacket()).getPingTime();
        final ServerPingTimeHandler handler2 = event.getSession()
            .getFlag("listener-ping-time-handler");
        if (handler2 != null) {
      ...
      if (event.getPacket() instanceof ServerKeepAlivePacket) {
        event.getSession().send(
            new ClientKeepAlivePacket(((ServerKeepAlivePacket) event.getPacket()).getPingId()));
      } else if (event.getPacket() instanceof ServerDisconnectPacket) {
        event.getSession()
            .disconnect(((ServerDisconnectPacket) event.getPacket()).getReason().getFullText());
      } else if (event.getPacket() instanceof ServerSetCompressionPacket) {
        event.getSession().setCompressionThreshold(
            ((ServerSetCompressionPacket) event.getPacket()).getThreshold());
      }
    }
  }
