...
public class AESEncryption implements PacketEncryption {
  ...
  public AESEncryption(final Key key) throws GeneralSecurityException {
    (this.inCipher = Cipher.getInstance("AES/CFB8/NoPadding"))
        .init(2, key, new IvParameterSpec(key.getEncoded()));
    (this.outCipher = Cipher.getInstance("AES/CFB8/NoPadding"))
        .init(1, key, new IvParameterSpec(key.getEncoded()));
  }
  ...
  @Override
  public int decrypt(final byte[] input, final int inputOffset, final int inputLength,
      final byte[] output, final int outputOffset) throws Exception {
    return this.inCipher.update(input, inputOffset, inputLength, output, outputOffset);
  }

  @Override
  public int encrypt(final byte[] input, final int inputOffset, final int inputLength,
      final byte[] output, final int outputOffset) throws Exception {
