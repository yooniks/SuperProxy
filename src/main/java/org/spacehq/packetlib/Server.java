...
public class Server {
  ...
  public Server(final String host, final int port, final Class<? extends PacketProtocol> protocol,
  ...
  public PacketProtocol createPacketProtocol() {
    try {
      final Constructor<? extends PacketProtocol> constructor = this.protocol
          .getDeclaredConstructor((Class<?>[]) new Class[0]);
      if (!constructor.isAccessible()) {
        constructor.setAccessible(true);
      }
      return (PacketProtocol) constructor.newInstance(new Object[0]);
    } catch (NoSuchMethodError e2) {
      throw new IllegalStateException("PacketProtocol \"" + this.protocol.getName()
          + "\" does not have a no-params constructor for instantiation.");
    } catch (Exception e) {
      throw new IllegalStateException(
          "Failed to instantiate PacketProtocol " + this.protocol.getName() + ".", e);
    }
  }
