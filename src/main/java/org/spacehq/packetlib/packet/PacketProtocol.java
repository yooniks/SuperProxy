...
public abstract class PacketProtocol {
  ...
  public final Packet createIncomingPacket(final int id) {
    ...
    try {
      final Constructor<? extends Packet> constructor = packet
      ...
      throw new IllegalStateException("Packet \"" + id + ", " + packet.getName()
          + "\" does not have a no-params constructor for instantiation.");
    } catch (Exception e) {
      throw new IllegalStateException(
          "Failed to instantiate packet \"" + id + ", " + packet.getName() + "\".", e);
    }
  }
